Модифицированная версия генератора отчета для расчета налогов с прибыли в Interactive Brokers для России
- использует `uv` для управления зависимостями
- не использует отчеты settle date
- игноирует ошибки при сплите и отзыве дивидендов
- генерирует отчет в HTML вместо PDF чтобы не устанавливать `pango`

**Пример использования:**
```sh
uv run -m investments.ibtax --activity-reports-path 'ibkr-*.csv' --year 2024 --save-to report2024.html
```
